import { AddCardData, EditCardData, FormDataFields, ICard } from "../types/types";
...
export function cloneTemplate<T extends HTMLElement>(query: string | HTMLTemplateElement): T {
    ...
    if (!el) {
        throw new Error('пустой template')
    }
...
export function normalizeFormData(
  ...
  return {
    ...
    status: raw.status?.toString() as ICard['status'],
    hours: Number(raw.hours) || 0,
    extra: raw.extra?.toString() || '',
    imageFile: raw.image instanceof File ? raw.image : undefined,
  };
}